        ...
        >
          ...
          <el-table-column prop="type_name" label="Name" align="center">
            <template slot-scope="scope">
              <el-input
                v-if="editing.status && scope.row._id === editing._id"
                v-model="editedType"
...
export default {
  ...
  computed: {
    ...mapState('newProduct', ['product_name', 'types'])
  },
  mounted() {
    setTimeout(() => {
  ...
  methods: {
    ...
    editThis(row) {
      ...
      this.editing._id = row._id
      this.editedType = row.type_name
    },
    saveEdit(name) {
      this.editing.status = false
      const index = this.types_.findIndex(
        (item) => item.type_name.toLowerCase().trim() === name.toLowerCase().trim()
      )
      this.types_[index].type_name = this.editedType + ''
      this.editedType = ''
      this.$store.commit('newProduct/SET_TYPES', this.types_)
    }
    // deleteThis(name) {
